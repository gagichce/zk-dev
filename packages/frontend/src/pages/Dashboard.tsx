...
export default observer(() => {
  ...
  const fieldLabel = (i: number, includeType: boolean = false) => {
    if (i < dataLabels.length) return dataLabels[i]

    let ret = `Data ${i}`
    if (includeType) ret += ` (${fieldType(i)})`
    return ret
  }
      ...
      <div className="container">
        ...
        <div style={{ display: 'flex' }}>
          ...
          <div className="action-container">
            ...
            {Array(userContext.userState.sync.settings.sumFieldCount)
              .fill(0)
              .map((_, i) => {
                return (
                  <div key={i} style={{ margin: '4px' }}>
